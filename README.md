...
- [Планер дня, план из личной базы знаний](#планер-дня-план-из-личной-базы-знаний)
...
## Планер дня, план из личной базы знаний

*Каждое утро собирает план дня из личной базы знаний и присылает его в Telegram: фокус, приоритеты, риски.*

### Что это

Планер дня читает личную базу знаний (заметки и карточки проектов, которые хранятся в отдельном репозитории) и раз в утро собирает из нее короткий план на день. Искусственный интеллект просматривает общую карту дел и карточки проектов, а затем формирует понятную сводку: на чем сфокусироваться, что в приоритете, что заблокировано и чего пока не трогать.

### Проблема, которую решает

Задачи и заметки копятся в разных карточках, и утром надо самому вспоминать, что важно именно сегодня, что горит, а что заморожено. На это уходят силы и внимание еще до начала работы. Хочется, чтобы кто-то за ночь пробежался по всем записям и выдал готовый план, а не сырой список дел.

### Как это работает

1. Планировщик запускается каждое утро.
2. Автоматизация забирает из личной базы знаний общую карту дел и все карточки проектов.
3. Содержимое собирается в один запрос вместе с текущей датой.
4. Искусственный интеллект по строгой структуре формирует план: фокус дня, приоритеты, риски и блокеры, а также то, что заморожено и не стоит начинать.
5. Готовый план приходит в Telegram аккуратным сообщением.

```mermaid
flowchart TD
    A[Планировщик, каждое утро] --> B[Чтение карты дел и карточек проектов из базы знаний]
    B --> C[Сборка контекста в один запрос с датой]
    C --> D[ИИ формирует план: фокус, приоритеты, риски, заморожено]
    D --> E[Отправка плана в Telegram]
```

### Почему так устроено

База знаний хранится в обычных текстовых заметках в отдельном репозитории, поэтому автоматизации достаточно просто прочитать их через API, ничего экспортировать вручную не нужно. Структура плана задана жестко в промпте, из-за этого сводка каждый день выглядит одинаково предсказуемо: одни и те же разделы в одном и том же порядке. Если по какому-то разделу данных нет, он просто пропускается, чтобы сообщение не разрасталось пустотой.

### Польза и результат

Утро начинается не с попытки вспомнить все дела, а с готового плана в мессенджере. Видно главное на сегодня, что в приоритете и что заблокировано, а лишнее и замороженное не отвлекает. Несколько минут на прочтение вместо ручной разборки заметок.
...
- `workflows/daily-planner.json`, план дня из личной базы знаний с отправкой в Telegram.
